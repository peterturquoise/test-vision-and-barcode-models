...
func main() {
	fmt.Println("🚀 C++ ZXing CGO Client - Testing memory-only processing")
	...
	fmt.Println("🔄 Testing CGO + OpenCV + ZXing-CPP memory processing...")
	...
	case d := <-msgs:
		...
		fmt.Println("🎉 CGO memory processing successful!")
	case <-time.After(30 * time.Second):
		fmt.Println("⏰ Timeout waiting for result")
	}

	fmt.Println("✅ CGO Client test complete!")
}
